...
class UncensoredLLM(commands.Cog):
    ...
    async def _handle_chat(self, ctx, message: str):
        ...
        try:
            ...
            if len(history) > max_len:
                ...
                await ctx.send(
                    "Conversation history was trimmed to stay within the max length."
                )
            ...
            await ctx.send(
                f"Cannot connect to LLM server at the configured address.\n"
    ...
    async def sethost(self, ctx, host: str):
        ...
        await ctx.send(f"Host updated to `{host}` (visible only to owner)")
