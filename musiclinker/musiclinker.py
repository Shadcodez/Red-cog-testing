...
class MusicLinker(commands.Cog):
    """Detects Spotify, YouTube, Amazon Music, and Tidal music links, then replies with
    cross-platform search links and a Brave Search lyrics link.
    ...
    - Support for Spotify, YouTube, Amazon Music, and Tidal links
    """

    SPOTIFY_GREEN = 0x1DB954
    YOUTUBE_RED = 0xFF0000
    AMAZON_BLUE = 0xFF9900
    TIDAL_BLACK = 0x000000
    ...
    # Matches Amazon Music links
    AMAZON_RE = re.compile(
        r"https?://(?:music\.)?amazon\.(?:com|co\.uk|de|fr|es|it|ca|in|com\.br|com\.mx|co\.jp|com\.au)/"
        r"(?:albums/|tracks/)?([a-zA-Z0-9]+)(?:\?|/|\s|$)"
    )

    # Matches Tidal links (tracks and albums)
    TIDAL_RE = re.compile(
        r"https?://(?:www\.)?tidal\.com/(?:browse/)?track/(\d+)(?:\?|/|\s|$)"
    )
    ...
    def __init__(self, bot: Red):
        ...
        self._message_links: dict = {}  # {message_id: {"spotify": [...], "youtube": [...], "amazon": [...], "tidal": [...]}}
    ...
    async def _fetch_amazon_music_metadata(self, asin: str) -> dict | None:
        """Fetch basic metadata from Amazon Music via oEmbed (if available) or fallback."""
        # Amazon doesn't have a standard oEmbed endpoint, so we'll extract from the URL
        # In a real implementation, you might scrape the page or use an API
        # For now, we return a basic structure that will be used in searches
        return {
            "asin": asin,
            "track_name": f"Amazon Music Track ({asin})",
            "artists": "",
            "album": "",
            "source": "amazon",
        }
    ...
    async def _fetch_tidal_track(self, track_id: str) -> dict | None:
        """Fetch metadata from Tidal (using public data)."""
        session = await self._get_session()
        try:
            # Tidal has an undocumented API, but we can use basic info
            # For a production system, consider using Tidal's official API
            async with session.get(
    ...
    def _amazon_music_search(query: str) -> str:
        return f"https://music.amazon.com/search/{quote_plus(query)}"

    @staticmethod
    def _tidal_search(query: str) -> str:
        return f"https://tidal.com/search?q={quote_plus(query)}&types=TRACKS"
    ...
    def _build_spotify_embed(
        ...
        # For YouTube, Amazon, and Tidal we include the album to help narrow results
        full_query = " ".join([p for p in (artists, track_name, album) if p])
        lyrics_query = search_query  # artist + song is ideal for lyrics

        yt_music = self._yt_music_search(full_query)
        yt_regular = self._yt_search(full_query)
        amazon = self._amazon_music_search(full_query)
        tidal = self._tidal_search(full_query)
        ...
        embed.add_field(
            name="🟠  Amazon Music",
            value=f"[Search]({amazon})",
            inline=True,
        )
        embed.add_field(
            name="⚫  Tidal",
            value=f"[Search]({tidal})",
            inline=True,
        )
        embed.add_field(
            name="📝  Lyrics",
        ...
        if track_info["source"] == "oembed":
            embed.set_footer(
                text="MusicLinker  •  Spotify ➜ Cross-Platform  •  "
                "Tip: set Spotify API credentials for artist & album info"
            )
        else:
            embed.set_footer(text="MusicLinker  •  Spotify ➜ Cross-Platform")
        return embed
    ...
    def _build_youtube_embed(
        ...
        # Search queries for all platforms
        search_query = f"{artist} {song}" if artist else song
        spotify = self._spotify_search(search_query)
        amazon = self._amazon_music_search(search_query)
        tidal = self._tidal_search(search_query)
        brave = self._brave_lyrics(search_query)
        ...
        if show_thumb and thumbnail:
            embed.set_thumbnail(url=thumbnail)
        embed.set_footer(text="MusicLinker  •  YouTube ➜ Cross-Platform")
        return embed
    ...
    def _build_amazon_embed(
        ...
        """Build the reply embed for a detected Amazon Music link."""
        track_name = track_info["track_name"]
        artists = track_info["artists"]
        album = track_info["album"]
        ...
        # Search queries
        search_query = f"{artists} {track_name}" if artists else track_name
        full_query = " ".join([p for p in (artists, track_name, album) if p])
        ...
        embed = discord.Embed(
            title=f"🎵  {display_title}", color=self.AMAZON_BLUE
        )
        if album:
            embed.description = f"💿 *{album}*"
        ...
        embed.set_footer(text="MusicLinker  •  Amazon Music ➜ Cross-Platform")
        return embed

    def _build_tidal_embed(
        self,
        track_info: dict,
        show_thumb: bool,
    ) -> discord.Embed:
        """Build the reply embed for a detected Tidal link."""
        track_name = track_info["track_name"]
        artists = track_info["artists"]
        album = track_info["album"]
        ...
        embed = discord.Embed(
            title=f"🎵  {display_title}", color=self.TIDAL_BLACK
        )
        if album:
            embed.description = f"💿 *{album}*"
        ...
        embed.set_footer(text="MusicLinker  •  Tidal ➜ Cross-Platform")
        return embed
    ...
    async def musiclinker(self, ctx: commands.Context):
        ...
        **Subcommands:**
        • `toggle` - Toggle MusicLinker on or off for this server
        • `channel [channel]` - Set channel for MusicLinker (or all channels if not specified)
        • `react` - Toggle reaction mode (users click reaction to see embeds)
        • `thumbnail` - Toggle album/video thumbnail display
        • `maxlinks [1-10]` - Set max number of links to respond to per message
        • `spotifyapi <id> <secret>` - Set Spotify API credentials (owner only)
        • `clearapi` - Remove stored Spotify API credentials (owner only)
        
        **Supported Platforms:**
        🎵 Detects links from: Spotify, YouTube, Amazon Music, Tidal
        """
