...
class Excelembeds(commands.Cog):
    ...
    async def excelembed_guide(self, ctx: commands.Context):
        """Full detailed guide with explanations and examples."""
        embed = discord.Embed(
            title="Excelembeds – Full Guide",
            color=discord.Color.blue(),
            description="Upload a `.xlsx` file → rich embeds with images, buttons, dropdowns, mentions, and optional DM reminders.\n"
                        "One row = one embed. Get the template with `[p]excelembed template`"
        )
        ...
        embed.add_field(
            ...
            value=(
                ...
                "`content` — plain text shown above the embed (optional)\n"
                "`color` — `#hex` or color name (optional)\n"
        ...
        embed.add_field(
            ...
            value=(
                "`ping_role` — role ID → normal mention\n"
                "`silent_ping_role` — role ID → silent mention (no notification sound)\n"
        ...
        embed.set_footer(text="JSON columns must be valid JSON • Silent ping uses Discord allowed_mentions")
