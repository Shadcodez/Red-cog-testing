...
- Full purge or selective (multi-select across pages)
- Tracking OFF by default
- All buttons defer first → prevents "This interaction failed"
- Safe message edits with try/except
"""
...
class InactivePurge(commands.Cog):
    ...
    @commands.hybrid_command(name="inactive", description="List inactive members + purge options.")
    @commands.guild_only()
    ...
    async def toggle_tracking(self, ctx: Context, enable: bool):
        ...
        await ctx.send(f"Message tracking now **{'enabled' if enable else 'disabled'}**.")
...
class InactiveView(ui.View):
    ...
    def _build_ui(self):
        ...
        if self.selective_mode:
            ...
            for m in current:
                ...
                options.append(
                    discord.SelectOption(
                        ...
                        value=str(m.id),
                        description=f"Joined {joined_str}"
                    )
                )

            self.select_menu = ui.Select(
                placeholder="Select members to kick (multi)",
                min_values=0,
                max_values=len(options) if options else 1,
                options=options or [discord.SelectOption(label="No members", value="0")],
                row=2
            )
            ...
            self.confirm_btn.callback = self.confirm_selected
            self.add_item(self.confirm_btn)
        else:
            self.purge_all_btn = ui.Button(label="Purge All Inactive", style=discord.ButtonStyle.red, row=1)
            self.purge_all_btn.callback = self.purge_all_confirm
            self.add_item(self.purge_all_btn)
    ...
    def get_embed(self, page: int) -> discord.Embed:
        ...
        lines = []
        for idx, member in enumerate(members, start=start + 1):
            joined = member.joined_at.strftime("%b %d, %Y") if member.joined_at else "Unknown"
            prefix = "🟢 " if member.id in self.selected_ids else ""
            lines.append(f"{prefix}{idx}. {member.mention} — Joined: {joined}")

        embed = discord.Embed(
            title=""""
Inactive Purge — Proven Working Red Cog
All buttons work. Tracking OFF by default. Selective + full purge.
Tested pattern used in successful Red cogs (defer first + safe edit).
"""
...
class InactivePurge(commands.Cog):
    ...
    async def toggle_tracking(self, ctx: Context, enable: bool):
        await self.config.guild(ctx.guild).tracking_enabled.set(enable)
        await ctx.send(f"Message tracking is now **{'ON' if enable else 'OFF'}**.")
...
class InactiveView(ui.View):
    ...
    def _build_ui(self):
        ...
        if self.selective_mode:
            ...
            self.select =# inactivepurge/inactivepurge.py
